...
namespace Tools
{
    ...
    {
        public static void Open(object target, string name = null, bool privateMembers = false,
            bool newInstance = false, bool includeMethods = true)
        {
            ...
            window._includeMethods = includeMethods;
            window._path = null;
            window.Show(true);
        }
        ...
        private void OnGUI()
        {
            if (_path == null)
            {
                _path = new[] { Inspect(_name ?? "root", _target, _privateMembers, _includeMethods) };
            }
        ...
        private static PathValue Inspect(string targetName, object target, bool privateMembers, bool includeMethods)
        {
            ...
            if (target != null)
            {
                ...
                else
                {
                    ...
                    var path = new PathValue[fields.Length + property.Length + additional + 
                                             (includeMethods ? methods.Length : 0)];
                    var index = 0;
        ...
        private void AddPath(PathValue value)
        {
            if (value.value != null && !value.value.GetType().IsPrimitive && !(value.value is string))
            {
                ...
                _path[_path.Length - 1] = Inspect(value.name, value.value, _privateMembers, _includeMethods);
                _sortBy = SortBy.NAME_UP;
            }
        }
